...
//     {
//         handlerCalled = true;
//         Assert.Equal(new byte[] { 0xff, 0xff, 0x1337 }, name);
//         Assert.Equal(SExp.To(1337), args);
...
//     {
//         Console.WriteLine(OPERATOR_LOOKUP);
//         Assert.Equal(OPERATOR_LOOKUP(KEYWORD_TO_ATOM["+"], SExp.To(new List<int> { 3, 4, 5 }))[1], SExp.To(12));
//     }
//
//     [Fact]
...
//             Assert.Equal(DefaultUnknownOp(new byte[] { 0x3c }.Concat(suffix).ToArray(), SExp.Null()), Tuple.Create(61, SExp.Null()));
//         }
//     }
// }